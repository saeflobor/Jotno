...
export const addFamilyMember = async (req, res) => {
  ...
  if (!["father", "mother", "child", "sibling"].includes(relation))
  ...
  try {
    ...
    switch (relation) {
      ...
      case "child":
        ...
        break;
      case "sibling":
        if (!user.family.siblings.includes(member._id))
          user.family.siblings.push(member._id);
        if (!member.family.siblings.includes(user._id))
          member.family.siblings.push(user._id);
        break;
    }
  ...
  try {
    ...
    switch (relation) {
      ...
      case "child":
        ...
        break;
      case "sibling":
        user.family.siblings = user.family.siblings.filter(
          (id) => id.toString() !== memberId.toString()
        );
        member.family.siblings = member.family.siblings.filter(
          (id) => id.toString() !== userId.toString()
        );
        break;
    }
...
export const sendSosAlert = async (req, res) => {
  ...
  try {
    const user = await User.findById(userId)
      ...
      .populate(
        "family.father family.mother family.siblings family.children",
        "email username"
      );
    ...
    if (Array.isArray(user.family?.siblings))
      members.push(...user.family.siblings);
    ...
    return res
      .status(500)
      .json({ message: err.message || "Failed to send SOS" });
  }
};
